...
local RayfieldLibrary = {
	...
	Theme = {
		...
		Tek = {
			...
			SliderBackground = Color3.fromRGB(50, 138, 220),
			SliderProgress = Color3.fromRGB(0, 255, 209),
			SliderStroke = Color3.fromRGB(58, 163, 255),

			ToggleBackground = Color3.fromRGB(30, 30, 30),
			ToggleEnabled = Color3.fromRGB(0, 255, 225),
			ToggleDisabled = Color3.fromRGB(100, 100, 100),
			ToggleEnabledStroke = Color3.fromRGB(0, 170, 255),
			ToggleDisabledStroke = Color3.fromRGB(125, 125, 125),
			ToggleEnabledOuterStroke = Color3.fromRGB(100, 100, 100),
...
local customLoadingGui = game:GetObjects("rbxassetid://123920250308699")[1]
...
local loading = customLoadingGui:FindFirstChild("Loading")
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			--ThaytuberVN Hub:Notify({Title = "ThaytuberVN Hub Flags", Content = "ThaytuberVN Hub was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function openSearch()
	...
	TweenService:Create(Main.Search.UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {Transparency = 0}):Play()
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show ThaytuberVN Hub'.", Duration = 7, Image = 4400697855})
		else
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RayfieldLibrary:Notify({Title = 'Build Mismatch', Content = 'ThaytuberVN Hub may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of ThaytuberVN Hub is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if (Settings.KeySystem) then
		...
		if not Passthrough then
			...
			Rayfield.Enabled = false
			local KeyUI = useStudio and script.Parent:FindFirstChild('Key') or game:GetObjects("rbxassetid://86762802714764")[1]
...
Main.Search.Input.FocusLost:Connect(function(enterPressed)
    -- Removed auto-close on empty input
end)
...
function RayfieldLibrary:LoadConfiguration()
	...
	if CEnabled then
		...
		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "ThaytuberVN_Hub Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('ThaytuberVN_Hub Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "ThaytuberVN_Hub Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--RayfieldLibrary:Notify({Title = "ThaytuberVN_Hub Interface", Content = "Welcome to ThaytuberVN_Hub. These - are the brand new notification design for ThaytuberVN_Hub, with custom sizing and Rayfield calculated wait times.", Image = 4483362458})ThaytuberVN_Hub
